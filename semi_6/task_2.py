...
# +,-,/,* приоритет операций стандартный. * Добавьте скобки,  приоритет операций меняется.  ПАРСИНГ
# in -> 2 - 2 + 7 * 2
# out -> 14
# in -> 2 - (2 + 7) * 2
# out -> -16
...
actions = {
    "^": lambda x, y: str(float(x) ** float(y)), # словарь
...
def parse(exp_1):
    ...
    while i < len(exp_1):
        if exp_1[i] == "(":
            # Если дальше, с текущего индекса,есть ещё открывающая скобка вызываем рекурсивно parse
            if "(" in exp_1[i + 1:]:
